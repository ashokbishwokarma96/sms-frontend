...
class AddInventory extends Component {
  ...
  onPressDelete(inventory) {
    const { token, dispatch } = this.props;
    dispatch(deleteInventory({ token: token, inventory: inventory })).then(
      function (data) {
        dispatch(getInventories({ token: token }));
      }
    );
  ...
  render() {
    const {
      ...
      fetchingInventoriesError,
      deletingsInventoriesError,
        ...
        <Table.Row key={inventory.id}>
          ...
          <Table.Cell>
            <Icon
              name="trash outline"
              size="large"
              onClick={this.onPressDelete.bind(this, inventory)}
            />
            <Icon
              name="pencil"
              size="large"
              onClick={this.onPressEdit.bind(this, inventory)}
            />
          </Table.Cell>
        </Table.Row>
      );
